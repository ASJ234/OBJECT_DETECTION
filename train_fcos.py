...
def train():
    ...
    model = fcos_resnet50_fpn(weights=None, num_classes=3)
    model.to(device)
    ...
    print(f'[FCOS] Training complete. Best mAP: {best_map:.4f} at epoch {best_epoch}')
...
def run_xai():
    ...
    if not test_dataset:
        print('[FCOS] No test dataset for XAI.')
        return
